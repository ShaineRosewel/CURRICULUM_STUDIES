...
function(curr="elem", this="whole") {
  ...
  # get relevant data
  if (curr=="elem"){
    processed_curr<-readxl::read_xlsx("forprocessing/K12_math.xlsx", skip=3, n_max=1102, col_names = TRUE)
  } else if (curr=="jhs") {
    processed_curr<-readxl::read_xlsx("forprocessing/K12_math.xlsx", skip=1107, col_names = TRUE)
  } else if (curr=="k") {
    processed_curr<-readxl::read_xlsx("forprocessing/K12K_math.xlsx", skip=7, n_max=70)
  } else {
    print("wala sa choices yan - k, elem, jhs lang")
  }
  
  # removes columns with all NA
  processed_curr_wo_na <- processed_curr[, colSums(is.na(processed_curr)) != nrow(processed_curr)]
  ...
  slight_fix <- apply(processed_curr_wo_na[, weird_index], 2, function(x){names(which.max(table(x)))})
  x<-names(table(slight_fix))
  
  oo <- foreach::foreach(i=x, .combine=cbind) %do% {
    k <-append(processed_curr_wo_na[, which(colnames(processed_curr_wo_na)==i)],
        as.list(processed_curr_wo_na[,names(which(slight_fix==i))]))
    k1=dplyr::coalesce(!!!k) #coalesce a list of vectors
    }
  
  colnames(oo) <- x
  a<- setdiff(colnames(processed_curr_wo_na)[-(grep(pattern = "...[0-9]+" , x = colnames(processed_curr_wo_na)))],
              colnames(oo))
  b<-which(colnames(processed_curr_wo_na)%in% a)
  fin<-cbind(processed_curr_wo_na[,b], oo)
  
  if (curr %in% c("elem", "jhs")) {
    col_order <- union(colnames(processed_curr_wo_na)[-(grep(pattern = "...[0-9]+" , x =
                                                               colnames(processed_curr_wo_na)))],colnames(oo))
    fin2 <- fin[, col_order]
  } else if (curr == "k") {
...
if(curr=="elem"){
  ...
  c_1 = stringr::str_extract_all(o, "MK[A-Z]+(-|_)00-[0-9]+")[[1]]
  } else {
...
if(curr %in% c("elem", "jhs")){
  a2 <- tibble::as.tibble(a1) %>% transform(SUBJECT=substr(CODE, 1, 1),
                                  GRADE = stringr::str_extract(CODE, pattern="[0-9]+"),
                                  CONTENT = stringr::str_extract(CODE, pattern="([A-Z]+){2}"),
                                  QUARTER = stringr::str_extract(CODE, pattern="([IV]+)|((0+){2})")#,
                                  #what1 = stringr::str_extract(CODE, pattern="[a-z]+"),
                                  #what2 = stringr::str_extract(CODE, pattern="[0-9]+(\\.[0-9])?$")
                             )
  ...
  b<-fin3 %>% 
    dplyr::filter(!is.na(`PERFORMANCE STANDARDS`) | !is.na(`CONTENT STANDARDS`)) %>%
  ...
  ps <- b %>% dplyr::group_by(PSID) %>% 
   ...
   cs_1 <- cs %>% dplyr::group_by(PSID) %>% dplyr::summarise(CS=paste(CS, collapse="_"))
   ps_1 <- ps %>% dplyr::group_by(PSID) %>% dplyr::summarise(PS=paste(PS, collapse="_"))
  ...
  a2 <- tibble::as.tibble(a1) %>% transform(SUBJECT=substr(CODE, 1, 1),
                                  ...
                                  CONTENT = c(rep("Logic (L)", 13), rep("Number and Number Sense (NNS)", 22), rep("Measurement (ME)",
                                  10), rep("Geometry (G)", 5), rep("Statistics and Probability (SP)", 4)),
                                  #unknown = stringr::str_extract(CODE, pattern="(?<=MK).*?(?=-|_)"),
  ...
  a8 <- a7 %>% tidyr::separate(col = LEARNING.COMPETENCY, into = c("NUMBER", "LC"), sep = "\\.", extra = "merge")
